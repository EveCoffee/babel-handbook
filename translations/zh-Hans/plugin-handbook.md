...
插件可以启用 [babylon plugins](https://github.com/babel/babylon#plugins)，以便用户不需要安装/启用它们。 这可以防止解析错误，而不会继承语法插件。
...
有几种主要的方法来测试babel插件：快照测试，AST测试和执行测试。 对于这个例子，我们将使用 [jest](http://facebook.github.io/jest/)，因为它支持盒外快照测试。 我们在这里创建的示例是托管在这个 [this repo](https://github.com/brigand/babel-plugin-testing-example).
...
Babel核心使用[similar approach](https://github.com/babel/babel/blob/7.0/CONTRIBUTING.md#writing-tests)去获取快照和执行测试。
...
> ***对于将来的更新，请关注 [@thejameskyle](https://twitter.com/thejameskyle) 和 [@babeljs](https://twitter.com/babeljs) 的Twitter。***